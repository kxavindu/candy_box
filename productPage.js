function renderProduct() {
        ...
        <div class="bottom-wrap-payment mt-3">
          <div class="d-flex justify-content-between align-items-start">
            <div>
              ...
              <span id="totalPrice" style="font-weight:bold; font-size:1rem; color:#555;"></span>
            </div>
  ...
  let quantity = 1;
  let selectedPackName = ''; // <-- Store selected pack name
  ...
  // If product has packOptions → dropdown
  if (product.packOptions && product.packOptions.length > 0) {
    let selectHTML = `<select id="packSelect" class="form-select form-select-sm" style="background-color:#ffe5e5; font-weight:bold;">`;
    product.packOptions.forEach(pack => {
      selectHTML += `<option value="${pack.price}" data-name="${pack.name}">${pack.name} - Rs.${pack.price}</option>`;
    });
    selectHTML += `</select>`;
    priceArea.innerHTML = selectHTML;

    const packSelect = document.getElementById("packSelect");
    currentPrice = parseFloat(packSelect.value);
    selectedPackName = packSelect.options[packSelect.selectedIndex].dataset.name;
    ...
    // Single price (no packs)
    currentPrice = parseFloat(product.price.replace('Rs.', '').trim());
    selectedPackName = '';
    priceArea.innerHTML = `<span id="priceDisplay" style="font-weight:bold; font-size:1.2rem;">Rs.${currentPrice}</span>`;
  }
  ...
  orderBtn.addEventListener("click", () => {
    ...
    const packText = selectedPackName ? `\n*Pack:* ${selectedPackName}` : '';

    const message = `${dateTime}\n\n` +
                    `*🛍️ Order Catalog*\n\n` +
                    `*1️⃣ Item* - ${product.name}${packText}\n` +
                    `*2️⃣ Quantity* - ${quantity}\n` +
                    `*3️⃣ Price per item* - Rs.${currentPrice}${messageText}\n\n` +
                    `*✨ Total: Rs.${total}*\n\n` +
  ...
  addToCartBtn.addEventListener("click", (e) => {
    ...
    const existingIndex = cart.findIndex(item => item.id === product.id && item.customMessage === messageBox.value && item.packName === selectedPackName);
    if (existingIndex !== -1) {
